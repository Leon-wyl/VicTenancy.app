...
module "api" {
  ...
  execution_role_arn       = data.terraform_remote_state.bootstrap.outputs.lambda_execution_role_arn_production
  runtime_secret_arn       = data.terraform_remote_state.bootstrap.outputs.runtime_secret_arn_production
...
module "agent_orchestration" {
  ...
  source_git_sha                = var.source_git_sha
  runtime_secret_arn            = data.terraform_remote_state.bootstrap.outputs.runtime_secret_arn_production
  api_execution_role_name       = data.terraform_remote_state.bootstrap.outputs.lambda_execution_role_name_production
  supabase_project_ref          = var.supabase_project_ref
  agent_runtime_mode            = var.agent_runtime_mode
  agent_runtime_invoke_url      = var.agent_runtime_invoke_url
  agent_runtime_execute_api_arn = var.agent_runtime_execute_api_arn
