...
| 14d  | []() | **Managed Supabase Environments and Database Delivery** — Separate staging and production Supabase Cloud projects. CI-driven migration promotion via GitHub Actions (validate, deploy-staging, deploy-production). JWKS smoke verification. Backup/restore runbook with Free-plan limitations documented. Mark complete only after a successful staging promotion and protected production promotion. |
